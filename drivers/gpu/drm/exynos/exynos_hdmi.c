...
static void hdmi_reg_infoframes(struct hdmi_context *hdata)
{
	...
	if (!ret)
		ret = hdmi_avi_infoframe_finalize_and_pack(&frm.avi, buf, sizeof(buf));
	if (ret > 0) {
	...
	ret = drm_hdmi_vendor_infoframe_from_display_mode(&frm.vendor.hdmi,
			&hdata->current_mode);
	if (!ret)
		ret = hdmi_vendor_infoframe_finalize_and_pack(&frm.vendor.hdmi, buf,
				sizeof(buf));
	if (ret > 0) {
	...
	if (!ret) {
		...
		ret = hdmi_audio_infoframe_finalize_and_pack(&frm.audio, buf, sizeof(buf));
	}
	if (ret > 0) {
