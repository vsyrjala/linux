...
static int hdmi_avi_infoframe_config(struct sti_hdmi *hdmi)
{
	...
	ret = hdmi_avi_infoframe_finalize_and_pack(&infoframe, buffer, sizeof(buffer));
	if (ret < 0) {
...
static int hdmi_audio_infoframe_config(struct sti_hdmi *hdmi)
{
	...
	if (audio->enabled) {
		...
		ret = hdmi_audio_infoframe_finalize_and_pack(&audio->cea, buffer,
						sizeof(buffer));
		if (ret < 0) {
...
static int hdmi_vendor_infoframe_config(struct sti_hdmi *hdmi)
{
	...
	ret = hdmi_vendor_infoframe_finalize_and_pack(&infoframe, buffer, sizeof(buffer));
	if (ret < 0) {
