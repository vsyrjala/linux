					...
					struct drm_display_mode *mode)
{
	...
	err = hdmi_avi_infoframe_finalize_and_pack(&frame, buffer, sizeof(buffer));
	if (err < 0) {
					...
					const char *product)
{
	...
	err = hdmi_spd_infoframe_finalize_and_pack(&frame, buffer, sizeof(buffer));
	if (err < 0) {
...
static int mtk_hdmi_setup_audio_infoframe(struct mtk_hdmi *hdmi)
{
	...
	err = hdmi_audio_infoframe_finalize_and_pack(&frame, buffer, sizeof(buffer));
	if (err < 0) {
